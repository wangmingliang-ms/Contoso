...
public class DefaultRedisQueue implements SeimiQueue {
    ...
    public void init() {
        ...
        if (!CrawlerCache.isSpringBoot()) {
            SeimiConfig contConfig = CrawlerCache.getConfig();
            if (contConfig != null && contConfig.isEnableRedissonQueue()) {
                //直接启动的方式
                isEnableRedissonQueue = true;
                if (contConfig.getBloomFilterExpectedInsertions() > 0) {
                    expectedInsertions = contConfig.getBloomFilterExpectedInsertions();
                }
                if (contConfig.getBloomFilterFalseProbability() > 0) {
                    falseProbability = contConfig.getBloomFilterFalseProbability();
                }
            }
        } else {
